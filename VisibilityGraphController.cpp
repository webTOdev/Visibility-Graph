		...
		//  sweepLine=new Line(ori->x,ori->y,w_i->x,w_i->y);
		  ...
		  if(isVisible(w_i,ori,sweepLine,w_i_1,index,edges,bstEdgeList))
		  {
			 ...
			 // std::cout<<"Edges at w_i "<<w_i->id<<" ->"<<es[0]<<","<<es[1]<<std::endl;
			  ...
			  bstEdgeList= updateVectorEdgeList(bstEdgeList,otherEnds,es,ori,w_i);
...
deque<Line*> VisibilityGraphController::updateVectorEdgeList(deque<Line*> bstEdgeList,int* otherEnds,int* es,Point* ori,Point* w_i){

	double infinity=-9999999;
	double angle[2]={infinity,infinity};
	Line* ln[2];
	...
	for(int i=0;i<2;i++){
				 ...
				 // double dist=bg::distance(boost::make_tuple(c->x, c->y),w_i->p);
				  ln[i] = searchLineContainingPoint(c,es,visGraph->obsSides);
			//	  if(isRotationClockwise(ori,w_i,c)){
				  if(isLeft(ori,w_i,c)){
					  //CONFUSED ABOUT DIST currently storing as origin to obsSide distance
					  tLinestring lineS=createLineString(ln[i]);
					  std::cout<<"Line "<<ln[i]->id<<" and Point "<<c->x<<","<<c->y<< " is at clockwise side of Point "<<ori->id<<","<<w_i->id<<std::endl;
					  double dist=bg::distance(boost::make_tuple(ori->x, ori->y),lineS);
					  std::cout<<"Distance : "<<ln[i]->id<<" and "<<ori->id<<" : "<<dist<<std::endl;
					  //edges = insertOneEdgeInEdgeList(edges,ln[i],dist,ori,w_i,c);
					//  bstEdgeList.push_back(ln);
					  angle[i]=angleBetweenThreePoint(ori->x,ori->y,w_i->x,w_i->y,c->x,c->y);

				  }else{
					  std::cout<<"Line "<<ln[i]->id<<" and Point "<<c->x<<","<<c->y<<" is at anti-clockwise side of "<<ori->id<<","<<w_i->id<<std::endl;
					//  edges = eraseOneEdgeFromEdgeList(edges,ln[i]);
					  bstEdgeList=eraseOneEdgeFromVectorEdgeList(bstEdgeList,ln[i]);
				  }

	}
	 std::cout<<"Angle[0] "<<angle[0]<<" Angle 1"<<angle[1]<<std::endl;
	if(angle[0]==infinity && angle[1]==infinity){ //Both the edge were at counter clockwise
		 std::cout<<"Both anti-clock"<<std::endl;
		return bstEdgeList;
	}
	else
	if(angle[0]!=infinity && angle[1]!=infinity){
		 std::cout<<"Both clock"<<std::endl;
		//Bothe edge were  clockwise so we need to decide which edge to be in the top of list
		if(angle[0]>angle[1]){
			bstEdgeList.push_back(ln[0]); //As ln[1] has smaller angle with the origin so it will in the top of edge list
			bstEdgeList.push_back(ln[1]);
		}
		else{
			bstEdgeList.push_back(ln[1]);
			bstEdgeList.push_back(ln[0]);
		}
	}
	else
		if(angle[0]!=infinity){
